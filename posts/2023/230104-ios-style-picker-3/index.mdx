...
[이전 글들](https://velog.io/@ricale/series/iOS-%EC%8A%A4%ED%83%80%EC%9D%BC-Date-Picker-%EB%9D%BC%EC%9D%B4%EB%B8%8C%EB%9F%AC%EB%A6%AC-%EA%B5%AC%ED%98%84)에서는 TypeScript 와 React 로 _iOS 스타일 Date Picker_ 를 구현했고, 구현한 컴포넌트를 라이브러리로 만들어 NPM 에 배포했다.
