...
[이전 글](https://velog.io/@ricale/iOS-%EC%8A%A4%ED%83%80%EC%9D%BC-Date-Picker-%EA%B5%AC%ED%98%84-1.-%EA%B8%B0%EC%A1%B4-%EC%BD%94%EB%93%9C-%EB%A6%AC%EB%B7%B0)에서는 iOS 스타일 Date Picker 를 순수 자바스크립트로 구현한 오픈소스 코드 조각에 대해 코드 리뷰를 했다. 이후 (글로 정리하지는 않았지만) 약간의 리팩토링을 거쳐 해당 오픈소스를 React 컴포넌트로 구현했다.
