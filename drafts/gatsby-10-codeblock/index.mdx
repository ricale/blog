...
이 글에서 코드블록은 마크다운에서 아래와 같은 문법을 썼을 때
...
마지막으로 테마도 `gatsby-browser.ts` 에서 적용하자.
...
### 2.2.1. `gatsby-remark-code-titles` 플러그인
...
```ts{13-14}:title=gatsby-config.ts
// ...
  ...
  plugins: [
    // ...
    {
      ...
      options: {
        ...
        gatsbyRemarkPlugins: [
          // ...
          // 다른 코드블록 플러그인 (이를테면 `gatsby-remark-prismjs`) 보다 앞에 두자
          {
...
```js:title=SampleComp.tsx&hideCopyButton=true
import React from "react";
...
제목이 밋밋해보인다면 스타일을 수정해서 예쁘게 꾸미자

### 2.2.1. `gatsby-remark-code-titles` 플러그인 수정해서 사용
...
이렇게 코드블록 제목과 본문을 같이 어우르는 컨테이너가 있어야 코드블록 제목에 `position: absolute;` 스타일을 달아서 내가 원하는 모양으로 만들 수 있다.
...
```scss:title=src/components/MdxContent.tsx
...
### 2.3.1. `gatsby-remark-code-buttons` 플러그인
...
공식 문서에는 MDX 를 지원하지 않는다고 적혀있지만 ("This plugin doesn't support MDX. Example of MDX copy button.") 실제로는 문제 없이 동작한다.

스타일은 직접 (관련 엘리먼트들의 클래스 이름을) 지정할 수 있는 많은 옵션을 제공하므로, 플러그인의 공식 문서를 참고하자.

### 2.3.2. `gatsby-remark-code-buttons` 플러그인 수정해서 사용
...
(`gatsby-remark-code-buttons` 의 기본 기능에 만족한다면 이 섹션은 넘어가도 된다.)
...
- 해당 플러그인에는 엘리먼트의 클래스 이름 및 문구들을 직접 지정할 수 있는 옵션이 많이 었었지만, 나 혼자 쓸 플러그인에 그런 기능은 필요 없으므로 모두 덜어냈다.
- 해당 플러그인에서는 복사 기능이 `document.execCommand('copy');` API 를 사용해 구현되었는데, 이건 근래에 [권장되는 방식이 아니므로](https://developer.mozilla.org/en-US/docs/Web/API/Document/execCommand) `navigator.clipboard` API 를 사용해 구현했다.
...
이로서 코드블록 관련 기능을 구현해보았다. 다음에 정리할 것은 "GitHub Pages 배포" 정도만 남은 것 같다.
